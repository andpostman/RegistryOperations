...
class PackageRowRepositoryTest {
    ...
    @Test
    @DisplayName("Проверка запроса не существуещем значением в таблице БД")
    void whenTryToFindFromDbPackageRow_returnEmptyResult(){
        Flux<PackageRow> aNewPackage = repository.findByReqId(-1);
        StepVerifier
                .create(aNewPackage)
                .recordWith(ArrayList::new)
                .thenConsumeWhile(x -> true)
